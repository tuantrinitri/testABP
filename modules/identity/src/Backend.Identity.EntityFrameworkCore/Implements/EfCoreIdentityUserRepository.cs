...
public class EfCoreIdentityUserRepository : EfCoreRepository<IIdentityDbContext, IdentityUser, Guid>, IIdentityUserRepository
{
    ...
    {
        return (await GetQueryableAsync()).AsNoTracking().IncludeDetails();
    }
    ...
    public override async Task<IdentityUser> FindAsync(Guid id, bool includeDetails = true, CancellationToken cancellationToken = new())
    {
        return includeDetails
            ...
            : await (await GetQueryableAsync()).AsNoTracking().OrderBy(e => e.Id).FirstOrDefaultAsync(e => e.Id.Equals(id), GetCancellationToken(cancellationToken));
    }
}
