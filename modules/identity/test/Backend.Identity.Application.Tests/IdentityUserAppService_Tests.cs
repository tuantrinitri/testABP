...
public class IdentityUserAppService_Tests : IdentityApplicationTestBase
{
    ...
    public IdentityUserAppService_Tests()
    {
        ...
        _profileRepository = GetRequiredService<IUserProfileRepository>();
    }
    ...
    public async Task UpdateAsync()
    {
        ...
        var profile = (await _profileRepository.GetListAsync()).Find(x => x.UserId == johnNash.Id);

        var profileId = profile?.Id ?? Guid.NewGuid();
        ...
        var newProfile = await _profileRepository.GetAsync(profileId);
        newProfile.SoldierCode.ShouldBe("0001");
    }
