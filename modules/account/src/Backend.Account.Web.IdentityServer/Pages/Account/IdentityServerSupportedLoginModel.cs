...
public class IdentityServerSupportedLoginModel : LoginModel
{
    ...
    public override async Task<IActionResult> OnGetAsync()
    {
        ...
        if (context != null)
        {
            // Find a proper cancel way.
            // ShowCancelButton = true;

            LoginInput.UserNameOrEmailAddress = context.LoginHint;

            //Reference AspNetCore MultiTenancy module and use options to get the tenant key!
    ...
    public override async Task<IActionResult> OnPostAsync(string action)
    {
        ...
        //Find a way of getting user's id from the logged in user and do not query it again like that!
        var user = await UserManager.FindByNameAsync(LoginInput.UserNameOrEmailAddress) ??
                   await UserManager.FindByEmailAsync(LoginInput.UserNameOrEmailAddress);

        Debug.Assert(user != null, nameof(user) + " != null");
        await IdentityServerEvents.RaiseAsync(new UserLoginSuccessEvent(user.UserName, user.Id.ToString(), user.UserName)); //Use user's name once implemented
